...
from app.api.helpers.article_query_maker import filter_by_status, \
    filter_by_tags, filter_by_search_phrase, apply_sorting, apply_pagination
from app.api.helpers.add_article_methods import process_manual_entry, \
    process_url_entry, add_to_tags, process_file, process_file_upload
...
def add_article():
    ...
    try:
        ...
        article = add_to_tags(article=article, user_id=token_auth.current_user().id, tags=tags)
...
def file_uploaded(article_id):
    ...
    task = token_auth.current_user().launch_task('bg_add_article', article_id=article_id, file_ext=file_ext, file=None)
