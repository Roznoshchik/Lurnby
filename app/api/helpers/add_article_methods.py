...
def process_url_entry(article, url):
  ...
  if not validators.url(url):
    raise SuppliedDataException("Can't validate url. Please check the data and try again")
...
def process_file(article=None, file=None, user=None):
  ...
  if file_ext != '.epub' and file_ext != '.pdf':
    ...
    raise SuppliedDataException('File must be pdf or epub')
      ...
      task = user.launch_task('bg_add_article', article_id=article.id, file_ext=file_ext, file=file)
...
def process_file_upload(article, file_ext):
  ...
  Returns:
      response object: status_code = 201, json={upload_url:str, location: str, file_ext:str, article_id: int, processing:bool}
  """
