...
def extract_title(html):
    ...
    extracted_titles = extract_element(
        html, xpaths, process_dict_fn=combine_similar_titles
    )
...
def combine_similar_titles(extracted_strings):
    ...
    for title_pair in permutations(extracted_strings, 2):
        # If the first title is a subset of the second then combine
        # their scores, taking the shorter one as the key
        if title_pair[0] in title_pair[1]:
            extracted_strings[title_pair[0]]["score"] += extracted_strings[
                title_pair[1]
            ]["score"]

            extracted_strings[title_pair[0]]["xpaths"] += extracted_strings[
                title_pair[1]
            ]["xpaths"]
        # If the first title is identical to the second (ignoring case)
        # then combine their scores, taking the one with more capitals as
        # the key
        elif title_pair[0].lower() == title_pair[1].lower():
            if len([c for c in title_pair[0] if c.isupper()]) > len(
                [c for c in title_pair[1] if c.isupper()]
            ):
