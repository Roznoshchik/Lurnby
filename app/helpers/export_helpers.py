...
def get_highlights_export(highlights: list, path:str, ext:str):
    ...
    Args:
        highlights (list of highlights): highlights
        path (str): path to temp dir
        ext (string): 'csv', 'json', or 'txt'
    """
    try:
...
def export_to_json(path, article_dict, highlights):
    ...
    with open(highlights_path, "w") as file:
        file.write(json.dumps(highlights, indent=4))

    return article_path, highlights_path


def export_to_txt(path, article_dict, highlights):
...
def export_to_csv(path, article_dict, highlights):
    ...
    with open(highlights_path, "w", newline="") as csvfile:
        if len(highlights):
            writer = csv.DictWriter(csvfile, highlights[0].keys())
            writer.writeheader()
            for highlight in highlights:
