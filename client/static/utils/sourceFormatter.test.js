...
describe('getReadableSource', () => {
  ...
  describe('Clean names from DB', () => {
    ...
    it('should truncate long clean names with ellipsis', () => {
      expect(
        getReadableSource('This is a very long clean name that exceeds twenty characters'),
  ...
  describe('Real URLs from DB', () => {
    it('should handle New York Times URLs', () => {
      expect(
        getReadableSource(
          'https://www.nytimes.com/2024/10/07/opinion/novelist-back-to-school-behavioral-science-identity.html?unlocked_article_code=1.QU4.FJTD.Is5XRghY9vMt&mc_cid=13ca65e521&mc_eid=64d208ece4',
        ),
      ).toBe('Nytimes')
    })
    ...
    it('should handle Fortune URLs', () => {
      expect(
        getReadableSource(
          'https://fortune.com/longform/wnba-cathy-engelbert-nba-caitlin-clark-angel-reese-nike-basketball/?nba224=&utm_source=substack&utm_medium=email',
        ),
      ).toBe('Fortune')
    })
    ...
    it('should handle New Yorker URLs', () => {
      expect(
        getReadableSource(
          'https://www.newyorker.com/culture/open-questions/does-anyone-really-know-you?mc_cid=13ca65e521&mc_eid=64d208ece4',
        ),
      ).toBe('Newyorker')
    })
    ...
    it('should handle Reddit URLs', () => {
      expect(
        getReadableSource('https://www.reddit.com/r/ableton/comments/5rjqer/apc40_vs_apc40_mkii/'),
      ).toBe('Reddit')
    })
    ...
    it('should handle Patreon URLs', () => {
      expect(getReadableSource('https://www.patreon.com/posts/ratcheting-in-47976114')).toBe(
        'Patreon',
      )
    })
    ...
    it('should handle threadreader URLs', () => {
      expect(getReadableSource('https://threadreaderapp.com/thread/1379844845536223235.html')).toBe(
        'Threadreaderapp',
      )
    })
    ...
    it('should handle international domains', () => {
      expect(
        getReadableSource(
          'https://www.nexojornal.com.br/colunistas/2025/03/05/so-da-para-entender-trump-a-partir-do-conflito-com-a-china?utm_medium=email',
        ),
      ).toBe('Nexojornal')
      expect(
        getReadableSource(
          'https://www.masterstudies.com/Masters-Degree-Programme-in-Peace-Mediation-and-Conflict-Research/Finland/%C3%85bo-Akademi-University/',
        ),
      ).toBe('Masterstudies')
    })
  })
  ...
  describe('Real edge cases from DB', () => {
    ...
    it('should handle blog names', () => {
      ...
      expect(getReadableSource('Center for Progressive Reform blog')).toBe(
        'Center for Progressi...',
      )
    })
  })
})
