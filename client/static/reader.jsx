...
function ReaderPage() {
  ...
  const fetchArticle = async () => {
    try {
      ...
      if (isUnprocessed) {
        const { data } = await api.post(ROUTES.API.articleProcess(articleUuid), highlightUuid ? { highlight: highlightUuid } : {})
        ...
        if (data.unanchorable) {
          alert.warning("Sorry, this highlight couldn't be located in the article text.")
        }
        ...
        if (highlightUuid) {
          const { data: hData } = await api.get(ROUTES.API.highlight(highlightUuid))
          const highlight = hData?.highlight
  ...
  useEffect(() => {
    if (!article || hasJumpedRef.current || !initialPositionRef.current) return
  ...
  const handleToggleNotes = useCallback(() => {
    ...
    if (position) {
      setTimeout(() => {
        contentRef.current?.scrollToPosition({ chunk: position.chunk, offset: position.offset, behavior: 'instant' })
      }, 310)
    }
  }, [])
  ...
  return (
    <Layout sidebarContent={sidebarContent}>
      <div ref={readerContainerRef} className={`reader-wrapper ${notesOpen ? 'with-notes' : ''}`}>
        ...
        <Loader isLoading={loading} mode="overlay" text="Loading...">
          <ReaderContent
            ref={contentRef}
            title={article?.title}
            chunks={chunks}
